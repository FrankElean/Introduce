<template>
    <div @click.stop=""  @touchstart.stop="" class="calendar" v-show="show" :style="{'left':x+'px','top':y+'px'}" transition="calendar" transition-mode="out-in">
			<div class="calendar-tools">
					<i class="fa fa-angle-left float left" v-on:click="prev"  v-on:touchstart="prev">&lt;</i>
					<i class="fa fa-angle-right float right" v-on:click="next"  v-on:touchstart="next">&gt;</i>
					<div class="text center">
						<span v-text="year"></span> / {{month+1 | formatTime}}
					</div>
			</div>
			<table cellpadding="5">
					<thead>
                                ...
                                if (that.range) {
                                        ...
                                        if (
                                                ...
                                                if (that.begin != undefined) {
                                                        ...
                                                        var thisTime = Number(new Date(that.year, that.month, i));
                                                        if (beginTime > thisTime) options.disabled = false;
                                                }
